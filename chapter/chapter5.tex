...
In questo capitolo verrá analizzata una possibile applicazione in ambito industriale:
verrá utilizzato un sensore di temperatura e umiditá che preleva delle informazioni ad intervalli regolari e
sfrutteremo Redis per la persistenza, l'organizzazione dei dati e l'elaborazione di essi.
...
\begin{enumerate}
    ...
    \item metodo avanzato, dove viene utilizza una struttura piú complessa, non presente nelle strutture dati di base di Redis,
...
Di conseguenza, il server sará raggiungibile tramite indirizzo IP e Porta.\\
...
Per quanto riguarda la persistenza, possiamo lasciare quella base, che é una RDB.
...
Per quanto riguarda la connessione al Server Redis,  con \texttt{redisConn.connect(...)} viene creata una connessione con il server
...
void loop()
{
   ...
   delay(5000);
    ...
    una nuova temperatura bisognerà utilizzare un fattore moltiplicativo pari ai decimali presenti nella sua rilevazione, per non perdere informazioni sulla
    risoluzione dei valori rilevati.
    Bisognerà ricordarsi di tenerne conto nell'interpretazione dei valori presenti nel database, ad esempio quando
...
\end{lstlisting}
    ...
    \begin{itemize}
        ...
        all'interno della serie; i valori che hanno una differenza maggiore del range specificato come parametro in ingresso vengono eliminati.
...
Viene implementata l'interfaccia \texttt{Runnable} e viene fatto un override sul metodo \texttt{run()}.
